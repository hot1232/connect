...
var ErrServerClosed = errors.New("sshserver: Server closed")

var authmutex sync.Mutex
var authorisedKeys map[string]deviceInfo
...
type Server struct {
	...
	Handler     Handler  // handler to invoke, sshserver.DefaultHandler if nil
...
func (srv *Server) ensureHandlers() {
	...
	srv.PasswordHandler = PasswordCallback

	srv.PublicKeyHandler = PublicKeyCallback

	srv.loadHostKeys()

}

func (srv *Server)loadHostKeys() {
	private, err := gossh.ParsePrivateKey([]byte(conf.Setting.Sshd.HostKey))
	if err != nil {
		log.Fatalf("Failed to parse private key, err: %v, key content: %v",err,conf.Setting.Sshd.HostKey)
	}
