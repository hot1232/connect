/*
Package sshserver wraps the crypto/sshserver package with a higher-level API for building
SSH servers. The goal of the API was to make it as simple as using net/http, so
the API is very similar.

You should be able to build any SSH server using only this package, which wraps
relevant types and some functions from crypto/sshserver. However, you still need to
use crypto/sshserver for building SSH clients.
...
It's a better idea to generate or point to an existing key on your system:
  ...
  log.Fatal(sshserver.ListenAndServe(":2222", nil, sshserver.HostKeyFile("/Users/progrium/.sshserver/id_rsa")))
